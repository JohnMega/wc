...
float Triangle::GetMinDistSurfaceDot(COORDS camCoord) const noexcept
{
	//return min(dotDistance(camCoord, surfaceDot1), min(dotDistance(camCoord, surfaceDot2), dotDistance(camCoord, surfaceDot3)));
	return (camCoord.x - centreCoord.x) * (camCoord.x - centreCoord.x) + (camCoord.y - centreCoord.y) * (camCoord.y - centreCoord.y) + (camCoord.z - centreCoord.z) * (camCoord.z - centreCoord.z);
}
