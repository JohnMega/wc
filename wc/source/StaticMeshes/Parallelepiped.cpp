...
Parallelepiped::Parallelepiped(float length, float width, float height, COORDS centreCoord, bool IsRotateAroundZAxis, bool IsRotateAroundXAxis,
    ...
    , color(color)
    , anglesRotationSum(0)
    , parallelepipedParabMode{ .isParab = false, .isEnvParabMode = false, .parabRadius = 5, .parabOwnerRange = {0,0,0,0,0,0} }
...
float& Parallelepiped::GetLenght() noexcept
{
    return length;
}

float& Parallelepiped::GetWidth() noexcept
{
    return width;
}

float& Parallelepiped::GetHeight() noexcept
{
    return height;
}
...
bool Parallelepiped::IsHasCollision() const noexcept
{
    return isHasCollision;
}

void Parallelepiped::SetCollisionMode(bool isHasCollision) noexcept
{
    this->isHasCollision = isHasCollision;
}
...
float Parallelepiped::GetWidthTextureCount() const noexcept
{
    return textureParams.widthTextureCount;
}

float Parallelepiped::GetHeightTextureCount() const noexcept
{
    return textureParams.heightTextureCount;
}
...
bool Parallelepiped::IsDotInCameraParaboloidRadius(COORDS checkedDot, COORDS cameraCoord, COORDS verCamDir, float farCrossRayAndFigureValue)
{
    if (!(checkedDot.z >= parRange.zEnd - 0.01 && checkedDot.z <= parRange.zEnd + 0.01) || !parallelepipedParabMode.isEnvParabMode) return false;

    COORDS camRayEndDot = { verCamDir.x * (farCrossRayAndFigureValue + 1) + cameraCoord.x, verCamDir.y * (farCrossRayAndFigureValue + 1) + cameraCoord.y, verCamDir.z * (farCrossRayAndFigureValue + 1) + cameraCoord.z };

    COORDS topSurfaceDot1 = { groundParMainDot1.x + centreCoord.x, groundParMainDot1.y + centreCoord.y, parRange.zEnd };
    COORDS topSurfaceDot2 = { groundParMainDot2.x + centreCoord.x, groundParMainDot2.y + centreCoord.y, parRange.zEnd };
    COORDS topSurfaceDot3 = { groundParMainDot3.x + centreCoord.x, groundParMainDot3.y + centreCoord.y, parRange.zEnd };
    COORDS verCamDirDotOnSurface = GetCrossingSurfaceAndLineCoords(topSurfaceDot1, topSurfaceDot2, topSurfaceDot3, cameraCoord, camRayEndDot);
    parallelepipedParabMode.verCamDirDotProj = verCamDirDotOnSurface;

    if (verCamDirDotOnSurface.x >= parRange.xStart && verCamDirDotOnSurface.x <= parRange.xEnd
        && verCamDirDotOnSurface.y >= parRange.yStart && verCamDirDotOnSurface.y <= parRange.yEnd)
    {
        if (dotDistance(checkedDot, verCamDirDotOnSurface) < parallelepipedParabMode.parabRadius) return true;
    }
